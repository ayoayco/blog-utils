export function renderMentions(mentions, rootSelector, headingMap) {
  ...
  ["like-of", "repost-of", "bookmark-of", "mention-of"].forEach((type) => {
    ...
    if (mentionsOfType.length) {
      const avatarBlock = createAvatarBlock(mentionsOfType, headingMap[type]);
  ...
  ["in-reply-to"].forEach((type) => {
    ...
    if (replies.length) {
      ...
      const repliesWrapper = createRepliesBlock(replies, headingMap[type]);
